...
class Transpose:
    ...
    def max_transpose(self, func: Callable[[np.ndarray], Any] | None = None) -> tuple[Polygon, Affine, Any | None]:
        ...
        """
        # Vectorized via scipy.signal.fftconvolve. Equivalent to the previous
        # per-shift loop at valid shifts: by construction no NaN ever falls
        # under the watershed mask at a valid position, so nanmean reduces to
        # sum/count exactly — which is what the correlation computes. ~100×
        # faster on 800×400 data with a ~50×50 mask.
